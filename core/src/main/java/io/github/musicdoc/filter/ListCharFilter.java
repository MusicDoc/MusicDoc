package io.github.musicdoc.filter;

import java.util.Locale;

/**
 * An implementation of {@link CharFilter} that holds a {@link #getChars() list of characters} that are
 * {@link #isAccepting() accepted or rejected}.
 */
public class ListCharFilter implements CharFilter {
   ...
   * @param to the last of the {@link #getChars() characters} range. Has to be greater or equal to {@code from}. Be
   *        careful when providing non-ASCII characters as unicode range can consume lots of memory.
   * @param accepting the {@link #isAccepting() accepting flag}.
   */
  ...
  /**
   * @return {@code true} if this {@link ListCharFilter} is {@link #accept(char) accepting} the {@link #getChars()
   *         listed chars}, {@code false} otherwise (if rejecting them).
   */
   ...
   * @return a {@link ListCharFilter} {@link #isAccepting() accepting or rejecting} both this {@link #getChars()
   *         characters} and the given {@code moreChars}.
   */
  ...
  /**
   * @param filter the {@link ListCharFilter} with the {@link #getChars() characters} to join. WARNING: it will be
   *        ignored if this filter is {@link #isAccepting() accepting} or not.
   * @return a {@link ListCharFilter} {@link #isAccepting() accepting or rejecting} both this {@link #getChars()
   *         characters} and the characters from given {@code filter}.
   */
  ...
  /**
   * @param charsUpperCase the {@link Character#isUpperCase(char) upper case} characters to
   *        {@link CharFilter#accept(char) accept}.
   * @return {@link CharFilter} that only {@link CharFilter#accept(char) accepts} characters from the given
   *         {@link String} including their {@link String#toLowerCase(Locale) lower case} variants.
   */
   ...
   * @return {@link CharFilter} that {@link CharFilter#accept(char) accepts} all characters except the ones from the
   *         given array.
   */
