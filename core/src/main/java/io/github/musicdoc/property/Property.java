...
public interface Property<V> {
  ...
  /**
   * Adds a {@link PropertyChangeListener} which will be notified whenever the value of the {@code Property} changes. If
   * the same listener is added more than once, then it will be notified more than once. Also the same
   * {@link PropertyChangeListener} instance may be registered for different {@link Property properties}.
   * <p>
   * The {@code Property} stores a strong reference to the listener which will prevent the listener from being garbage
   * collected and may result in a memory leak. It is recommended to unregister a listener by calling
   * {@link #removeListener(PropertyChangeListener)} after use.
   *
   ...
   * <p>
   * If the given listener has not been previously {@link #addListener(PropertyChangeListener) registered} this method
   * will have no effect. Otherwise it will be removed. If it had been added more than once, then only the first
