...
public abstract class ToneNameStyle<P extends TonePitch> {
  ...
  protected ToneNameStyle() {
    ...
    this.signs = new String[] { getDoubleFlatSign(), getSingleFlatSign(), "", getSingleSharpSign(),
    getDoubleSharpSign() };
  }

  /**
   * @return a {@link Map} mapping the accidental signs to their {@link EnharmonicType}.
   */
  ...
  /**
   * @return the (regular) single flat sign of this style. Please note that the returned {@link String} may not be
   *         consistently the single flat sign suffix. E.g. textual name styles (German or Dutch) use "es" as single
   *         sharp sign but the flattened pitch of "A" is "As" rather than "Aes" and the flattened pitch of "E" is "Es"
   *         rather than "Ees".
   */
  ...
  /**
   * @return the (regular) double flat sign of this style. Please note that the returned {@link String} may not be
   *         consistently the double flat sign suffix. E.g. textual name styles (German or Dutch) use "eses" as double
   *         sharp sign but the double flattened pitch of "A" is "Ases" rather than "Aeses" and the double flattened
   *         pitch of "E" is "Eses" rather than "Eeses".
   */
  public abstract String getDoubleFlatSign();

  /**
   * @return the (optional) neutral sign used to dissolve enharmonic signs. Used for
   *         {@link TonePitchInternational#B_NEUTRAL}. Will be the empty {@link String} if not used by this style.
   */
  public abstract String getNeutralSign();

  /**
   * @param enharmonicType the {@link EnharmonicType}.
   * @return the suffix to append to the base pitch (e.g. A, B, C, D, E, F, G) as enharmonic signs of the given
   *         {@link EnharmonicType}.
   */
  ...
  public EnharmonicType getType(String name) {
    ...
    return this.signsMap.get(rest);
  }

  /**
   * @param name the {@link #getName() name} to check.
   * @param enharmonicType the {@link EnharmonicType}.
   * @return {@code true} if the name matches for the given {@link EnharmonicType}, {@code false} otherwise.
   */
  ...
  /**
   * @param step the {@link TonePitch#getStep() step} of the requested {@link TonePitch}. Should be in the range from
   *        {@code 0} to {@code 11}.
   * @return the {@link TonePitch} of this {@link TonePitch#getNameStyle() name style} with the given
   *         {@link TonePitch#getStep() step} or {@code null} if no such pitch exists (if step is not in the range from
   *         {@code 0} to {@code 11}).
   */
   ...
   * @return the {@link TonePitch} of this {@link TonePitch#getNameStyle() name style} with the given
   *         {@link TonePitch#getStep() step} and {@link TonePitch#getEnharmonicType() enharmonic type} or {@code null}
   *         if no such pitch exists (e.g. for step {@code 8} no pitch exists for {@link EnharmonicType#NORMAL},
   ...
   * @param step the {@link TonePitch#getStep() step} of the requested {@link TonePitch}. Should be in the range from
   *        {@code 0} to {@code 11}.
   * @param style the preferred {@link EnharmonicStyle} of the requested {@link TonePitch}. However, to ensure that a
   *        valid result can always be returned even if no {@link TonePitch} exists for the given {@code step} that has
   *        the given {@link TonePitch#getEnharmonicStyle() enharmonic style}, this argument is only used as a
   *        preference. If a the given {@link EnharmonicStyle} can be satisfied with a
   *        {@link EnharmonicType#getSignCount() single enharmonic sign}, such pitch will be returned, otherwise the
   *        {@link TonePitch#isNormal() normal} pitch will be returned.
   * @param nameCase the {@link TonePitch#getCase() case} of the requested {@link TonePitch}.
   * @return the {@link TonePitch} of this {@link TonePitch#getNameStyle() name style} with the given
   *         {@link TonePitch#getStep() step} and a single enharmonic sign of the given {@link EnharmonicStyle} or if
   *         not possible the {@link TonePitch#isNormal() normal} one.
   */
   ...
   *        {@link EnharmonicStyle#isFlat() flat} signs of a {@link io.github.musicdoc.harmony.key.MusicalKey}. Should
